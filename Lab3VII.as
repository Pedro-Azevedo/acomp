...
; DADOS DE JOGO
	VECSIZE 		EQU 	0007h       ;dimensao dos vetores
	NUM_COR			EQU		0006h		;numero de cores 
	NUM_JOGADAS 	EQU 	000ah       ;numero de jogadas possíveis (10)
...
validacao:		MOV		R1, M[RIGHT_POS] 		;Guarda o numero de cores certas na posicao certa
				...
				PUSH	R1
					CALL	ValidaJogadaB
												;Carregar as peças guardadas em memoria auxiliar temporariamente
fimjogada:		MOV		R7, R0
				MOV		R4, PecaA
				MOV		R5, SPecaA
...
StartTmp:		MOV 	R2, M[IO_STAT]
				...
				BR.Z 	StartTmp
				MOV 	R1, M[IO_READ]         
				CMP 	R1, '1'  				;Garantir que o número escolhido é maior ou igual a 1
				BR.N 	StartTmp 
				CMP 	R1, '4'                 ;Garantir que o número escolhido é menor ou igual a 4 
				BR.P 	StartTmp
				CMP 	R1, '3'                 ;Garantir que o número escolhido não é 3 
				BR.Z 	StartTmp
				SUB 	R1, 0030h               ;Conversão do caracter ASCII do número para o número em hexadecimal
				MOV 	M[CronTmp], R1  		;Guardar na variável CronTmp
				POP 	R2 
				POP 	R1 
				RET
;=================================================================================
...
StartClr:		MOV 	R2, M[IO_STAT]
				...
				BR.Z 	StartClr
				MOV 	R1, M[IO_READ]
				CMP 	R1, '4' 				;Garantir que o número escolhido é maior ou igual a 4
				BR.N 	StartClr
				CMP 	R1, '6'                 ;Garantir que o número escolhido é menor ou igual a 6
				BR.P 	StartClr
				SUB 	R1, 0030h   			;Conversão do caracter ASCII do número para o número em hexadecimal
				MOV 	M[NumPecas], R1  		;Guardar na variável NumPecas 
				POP 	R2
				POP 	R1
				RET
...
MediaJgds:	    PUSH 	R1
				...
				PUSH 	R6
				PUSH 	R7
				MOV  	R3, M[ContagemJogos]	;R3 guarda o número de jogos ganhos
				MOV  	R2, M[WinNmbr]     		;R2 guarda o número do jogador (para aceder ao vetor)
NotBetterSeg: 	MOV  	R1, M[ContaJogadas]   	;R1 guarda o número de tentativas que o jogador precisou para acertar na sequência
				MOV  	R4, SumNumJogadas       ;R4 guarda o endereço do vetor SumNumJogadas (posição 0)
				ADD  	R4, R2       			;R4 desloca-se para a posição "nº do jogador" do vetor 
				ADD  	M[R4], R1     			;Ao conteúdo dessa posição adicionamos o valor de R1  
				MOV  	R5, M[R4]   			
				DIV  	R5, R3  				;Dividimos essa soma pelo número de jogos ganhos 
				MOV  	R6, MediaNumJogadas     ;R6 guarda o endereço do vetor MediaNumJogadas(posição 0)
				ADD  	R6, R2       			;R6 desloca-se para a posição "nº do jogador" do vetor 
				MOV  	M[R6], R5               ;Guarda nessa posição o valor obtido para a média (guardado em R5)
				 								;Agora deve proceder-se ao cálculo do valor decimal da média 
				MOV  	R5, M[R4]               ;Começa-se por copiar novamente o valor da soma
				MOV  	R6, R5
				MOV  	R7, R0   				
Soma10:			ADD  	R5, R6 					;O valor é somado com ele próprio 10 vezes(equivalente a multiplicação por 10)
				INC  	R7
				CMP  	R7, 0009h
				BR.NZ 	Soma10
				MOV  	R7, M[ContagemJogos]    ;Guarda-se novamente o número de jogos ganhos 
				DIV  	R5, R7    				;Divide-se pelo número de jogos ganhos (guarda o resultado em R5)
				MOV  	R7, 000ah
				DIV  	R5, R7                  ; Divide-se o valor obtido por 10 (para guardar o último dígito (resto da divisão) em R7)
				MOV  	R6, MediaNumJogadasDec  ;Guarda-se o valor obtido na posição "nº do jogador" do vetor MediaNumJogadasDec
				ADD  	R6, R2
				MOV  	M[R6], R7
				POP 	R7
				POP  	R6
