...
; DADOS DE JOGO
	...
	NUM_JOGADAS	EQU	000Ah		;numero de tentativas versao standard na posicao errada
...
EcraInicialR13	STR	'<-- (1)Jogar					Multiplayer(M)-->', FIM_TEXTO	
EcraInicialM1	STR	'		    	BEM VINDO AO MODO MULTIPLAYER', FIM_TEXTO
EcraInicialMX	STR	'=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=',FIM_TEXTO
EcraInicialM2	STR	'Escolha o número de jogadores: ',FIM_TEXTO
...
MenuEscolhe:	PUSH	R1
		PUSH	R3
retesta:	INC 	R3   				;Incrementar esse "tempo" enquanto nao for premida uma tecla
		MOV	R2, M[IO_STAT]			;Detetar se foi premida uma tecla
		CMP	R2, R0   			;Se for, a comparacao com R0 vai ser diferente de zero
		BR.Z	retesta  			;Se nao for, continua a esperar
...
NumeroJogador:	PUSH	R1
		BR	first
renumjog:	INC	R3				;chamar função aparecer erro
first:		INC 	R3   				;Incrementar esse "tempo" enquanto nao for premida uma tecla
		MOV	R2, M[IO_STAT]			;Detetar se foi premida uma tecla
		CMP	R2, R0   			;Se for, a comparacao com R0 vai ser diferente de zero
		BR.Z	renumjog  			;Se nao for, continua a esperar
		INC 	R3 				;Incrementar esse tempo quando a tecla é pressionada
		MOV	R1, M[IO_READ]   		;Guardar o que foi lido
		CMP	R1,'2'
		BR.N	renumjog
...
inicio:         MOV     R1, SP_INICIAL   	
                MOV     SP, R1
                MOV 	R3, 0a0ah 			;Registo para guardar um "tempo" ficticio enquanto o jogo nao comeca 
                	CALL 	EscEcraInicial 		;Escrever o Menu Inicial
			CALL	MenuEscolhe		;R3 é utilizado neste menu que escolhe a opçao
